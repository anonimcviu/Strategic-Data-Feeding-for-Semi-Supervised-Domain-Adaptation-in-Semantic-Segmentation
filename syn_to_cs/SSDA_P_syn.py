...
def get_dataset(opts):
    ...
    if opts.test_only:
        ...
        return gta_train, gta_val,cs_unl,cs_lb,cs_val
...
def main():
    ...
    if opts.test_only:
        ...
        source_trainLoader = data.DataLoader(
            ...
            worker_init_fn=worker_init_fn,drop_last=True)  # drop_last=True to ignore single-image batches.
        ...
        target_trainLoader_unl = data.DataLoader(
            target_train_unl, batch_size=opts.batch_size, shuffle=True, num_workers=4,
            worker_init_fn=worker_init_fn,pin_memory=True,
            drop_last=True)  # drop_last=True to ignore single-image batches.

        target_trainLoader_lbl = data.DataLoader(
            target_train_lbl, batch_size=opts.batch_size, shuffle=True, num_workers=4,
            worker_init_fn=worker_init_fn,pin_memory=True,
            drop_last=True)  # drop_last=True to ignore single-image batches.

        target_valLoader = data.DataLoader(
            target_val, batch_size=opts.batch_size, shuffle=False, num_workers=4,
    ...
    len_training = max(len(source_trainLoader)+ len(target_trainLoader_lbl), len(target_trainLoader_unl))

    wandb.login(key="a340a005e504029f57729282bbe32a8795a65970")

    with wandb.init(project="Paper_syn", name=opts.run_name):
        total_err_s_clf = total_err_s_disc= total_err_t_lbl_clf= total_err_t_unl_disc= total_err_t_lbl_disc=total_clf_loss=total_disc_loss=interval_loss=0        
        total_it=0
        ...
        for e in range(opts.num_epoch):
            ...
            while cur_itrs < len_training:
                ...
                if total_it % math.ceil(len_training // len(target_trainLoader_lbl)) == 0 and cur_itrs!=0 and labeled_count<len(target_trainLoader_lbl): # take image from labeled target domain every len(labeled_target)
                    labeled_count+=1 
                ...
                total_loss= err_s_clf +  err_t_lbl_clf  + ((err_t_lbl_disc +  err_t_unl_disc +err_s_disc)/2)
                ...
                total_err_s_clf += err_s_clf
                total_err_s_disc += err_s_disc
                total_err_t_lbl_clf += err_t_lbl_clf
                total_err_t_lbl_disc += err_t_lbl_disc
                total_err_t_unl_disc += err_t_unl_disc

                total_clf_loss += err_s_clf + err_t_lbl_clf
                total_disc_loss +=((err_t_lbl_disc + err_t_unl_disc +err_s_disc)/2)
