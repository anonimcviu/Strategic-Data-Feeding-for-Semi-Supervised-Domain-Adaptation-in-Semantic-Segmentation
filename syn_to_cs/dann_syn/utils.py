...
class _SimpleSegmentationModel(nn.Module):
    def __init__(self, backbone, classifier,discriminator):
    ...
    def forward(self, x,alpha):
        ...
        features = self.backbone(x)
                ...
                # During training, return both classifier and domain outputs
        classifier_output,concat = self.classifier(features)
        classifier_output = F.interpolate(classifier_output, size=input_shape, mode='bilinear', align_corners=False)
        domain_output=self.discriminator(concat,alpha)
        return  classifier_output, domain_output
