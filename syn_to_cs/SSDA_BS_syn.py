...
def get_dataset(opts):
    ...
    if opts.test_only:
        ...
        return train_set_gta_with_zeros, gta_val,cs_unl,cs_lb_with_ones,cs_val
...
class BatchLevelShuffleDataLoader:
    """
    A wrapper that combines two dataloaders and performs batch-level shuffling.
    Each batch remains homogeneous (from either source or target).
    """
    def __init__(self, source_dataset, target_dataset, batch_size, num_workers=4):
        self.source_dataset = source_dataset
        self.target_dataset = target_dataset
        self.batch_size = batch_size
        self.num_workers = num_workers
        
        # Calculate number of batches in each dataset
        self.source_batches = len(source_dataset) // batch_size
        self.target_batches = len(target_dataset) // batch_size
        self.total_batches = self.source_batches + self.target_batches
        
        # Create individual dataloaders with sequential sampling
        self.source_loader = DataLoader(
            source_dataset, 
            batch_size=batch_size,
            sampler=SequentialSampler(source_dataset),
        ...
        # Prepare the batch indices for shuffling
        self._prepare_batch_indices()
    
    def _prepare_batch_indices(self):
        """Prepares and shuffles batch indices"""
        # Create batch indices for source (0) and target (1)
        source_indices = [(0, i) for i in range(self.source_batches)]
        target_indices = [(1, i) for i in range(self.target_batches)]
        
        # Combine and shuffle
        self.batch_indices = source_indices + target_indices
        np.random.shuffle(self.batch_indices)
    
    def __iter__(self):
        """Returns an iterator over shuffled batches"""
        self._prepare_batch_indices()  # Reshuffle for each epoch
        
        # Create iterators for both datasets
        source_iter = iter(self.source_loader)
        target_iter = iter(self.target_loader)
        
        for loader_idx, batch_idx in self.batch_indices:
            try:
                if loader_idx == 0:  # Source dataset
                    batch = next(source_iter)
                else:  # Target dataset
                    batch = next(target_iter)
                yield batch
            except StopIteration:
                # Handle the case where one iterator is exhausted
                # (shouldn't happen with sequential samplers unless drop_last=False)
                continue
...
def main():
    ...
    if opts.test_only:
        ...
        source_train, source_val, target_train_unl, target_train_lbl, target_val  = get_dataset(opts)

        source_valLoader = data.DataLoader(
            source_val, batch_size=opts.batch_size, shuffle=False, num_workers=4,pin_memory=True,
        ...
        combined_trainLoader = BatchLevelShuffleDataLoader(
                source_train, target_train_lbl, batch_size=opts.batch_size, num_workers=2)
        
        # Calculate the length of training iterations based on the new combined loader
        len_training = max(len(combined_trainLoader), len(target_trainLoader_unl))
            
        print("Dataset: %s, Source Train set : %d, Source Val set: %d, Target Train Unlabeled set : %d, Target Train Labeled set : %d, Target Val set : %d" %
            (opts.dataset, len(source_train), len(source_val),len(target_train_unl), len(target_train_lbl), len(target_val)))
    ...
    with wandb.init(project="Paper_syn", name=opts.run_name):
        total_err_s_clf = total_err_s_disc= total_err_t_unl_disc=total_clf_loss=total_disc_loss=interval_loss=0        
        total_it=0
        ...
        for e in range(opts.num_epoch):
            ...
            combined_iter = iter(combined_trainLoader)
            ...
            while cur_itrs < len_training:
                ...
                try:
                    source_image, source_label,domain_label = next(combined_iter)
                ...
                with autocast(dtype=torch.bfloat16):
                    ...
                    domain_label_s = torch.full((opts.batch_size , domain_output_s.size(2), domain_output_s.size(3)),domain_label[0].item()).float().to(device)
                ...
                with autocast(dtype=torch.bfloat16):
                    ...
                    err_t_unl_disc=criterion_disc(domain_output.squeeze(1), domain_label)
                ...
                total_loss= err_s_clf + ((err_s_disc  +  err_t_unl_disc)/2) 
                ...
                total_err_t_unl_disc += err_t_unl_disc.item()

                total_clf_loss += err_s_clf .item()
                total_disc_loss += (err_s_disc.item()+ err_t_unl_disc.item())/2
