...
def get_argparser():
    ...
    parser.add_argument("--itrs_goal", type=int, default=30e3,
                        help="epoch number (default: 30k)")
...
def main():
    ...
    if opts.test_only:
        ...
        test_loader = data.DataLoader(
            test_dst, batch_size=opts.val_batch_size, shuffle=False, num_workers=2, pin_memory=True)
        print("Dataset: %s,  Target Test set : %d" % (opts.dataset,len(test_dst)))
        ...
        print("Dataset: %s, Labled Train set : %d, Val set: %d" %
    ...
    if opts.test_only:
        model.eval()
        val_score = validate(opts=opts, model=model, loader=test_loader, device=device)
    ...
    with wandb.init(project="Paper", name=opts.run_name):
        ...
        for e in range(opts.num_epoch):    # cur_itrs < opts.total_itrs:
            # =====  Train  =====
            model.train()
            cur_itrs = 0
            ...
            for (images, labels) in train_loader:
                ...
                if (total_it) % opts.val_interval == 0:
                    ...
                    wandb.log({"mean_IOU_t": val_score['Mean IOU'],
                                "mean_acc_t":val_score['Mean ACC'],
                                "overall_acc_t": val_score['Overall ACC'],
                                #"overall_iou_t": val_score['Overall IOU'],
