...
def get_dataset(opts):
    ...
    if opts.test_only:
        ...
        train_labeled = ConcatDataset([train_set_gta_with_zeros, cs_lb_with_ones])

        return train_labeled, gta_val,cs_unl,cs_val
...
def main():
    ...
    if opts.test_only:
        ...
        source_train, source_val, target_train_unl, target_val  = get_dataset(opts)
        ...
        source_valLoader = data.DataLoader(
            source_val, batch_size=opts.batch_size, shuffle=False, num_workers=4,
        ...
        print("Dataset: %s, Source Train set : %d, Source Val set: %d, Target Train Unlabeled set : %d, Target Val set : %d" %
            (opts.dataset, len(source_train), len(source_val),len(target_train_unl), len(target_val)))
    ...
    len_training = max(len(source_trainLoader), len(target_trainLoader_unl))
    ...
    with wandb.init(project="Paper", name=opts.run_name):
        ...
        print("len(source_trainLoader)",len(source_trainLoader))
        ...
        for e in range(opts.num_epoch):
            ...
            while cur_itrs < len_training:
                ...
                try:
                    source_image, source_label,domain_label = next(source_iter)
                ...
                total_err_t_unl_disc += err_t_disc.item()
